...
from agents.orchestrator import queue_job, run_job
...
def telegram_status(conn: sqlite3.Connection, user_id: str) -> dict:
    identity = conn.execute("SELECT ci.display_name,ci.linked_at,ci.active_child_id,cp.name child_name FROM channel_identities ci LEFT JOIN child_profiles cp ON cp.id=ci.active_child_id WHERE ci.user_id=? AND ci.provider='telegram' AND ci.unlinked_at IS NULL", (user_id,)).fetchone()
    public_identity = {"display_name": identity["display_name"], "linked_at": identity["linked_at"], "active_child_id": identity["active_child_id"], "child_name": identity["child_name"]} if identity else None
    return {"configured": bool(settings.telegram_bot_token and settings.telegram_webhook_secret), "linked": bool(identity), "identity": public_identity, "bot_username": settings.telegram_bot_username}
...
def consume_link_code(conn: sqlite3.Connection, code: str, provider_user_id: str, chat_id: str, display_name: str | None) -> tuple[bool, str, int | None]:
    ...
    if existing:
        ...
        conn.execute("UPDATE channel_identities SET unlinked_at=NULL,display_name=?,linked_at=?,active_child_id=? WHERE id=?", (display_name, now, row["child_id"], identity_id))
    else:
        cur = conn.execute("INSERT INTO channel_identities(user_id,active_child_id,provider,provider_user_id,provider_chat_id,display_name,linked_at) VALUES(?,?,'telegram',?,?,?,?)", (row["user_id"], row["child_id"], provider_user_id, chat_id, display_name, now))
...
def handle_update(update: dict, db_path: Path | str | None = None) -> dict:
    ...
    with transaction(db_path, immediate=True) as conn:
        ...
        if text.startswith("/start "):
            code = text.split(maxsplit=1)[1].removeprefix("link_")
            ok, response, identity_id = consume_link_code(conn, code, provider_user_id, chat_id, display_name)
            ...
            response = "NutriShield membantu pencatatan keluarga dan informasi pangan terbatas. Bukan alat diagnosis atau pemberi dosis obat. Tautkan akun dari dashboard menggunakan kode sekali pakai."
            ...
            child = conn.execute("SELECT id FROM child_profiles WHERE id=? AND user_id=?", (identity["active_child_id"], identity["user_id"])).fetchone() if identity["active_child_id"] else None
            if not child:
                response = "Profil anak aktif belum dipilih. Buat kode taut baru dari dashboard setelah memilih profil anak."
                delivery = persist_and_send(conn, chat_id, response, identity["user_id"], identity["id"])
            ...
            child_id = child["id"]
            job_id = queue_job(conn, identity["user_id"], child_id, "question_answer", "telegram.message", {"message": text, "channel": "telegram", "chat_id": chat_id, "channel_identity_id": identity["id"]})
