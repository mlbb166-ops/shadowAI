...
from agents.orchestrator import BackgroundWorker, queue_job, run_job, run_payload, search_foods
from .config import settings
from .db import IntegrityError, connect, initialize_database, transaction
from .models import ChatRequest, ChildCreate, LoginRequest, MeasurementCreate, RegisterRequest, TelegramLinkCodeRequest
...
def create_app(db_path: Path | str | None = None, *, start_worker: bool | None = None) -> FastAPI:
    actual_db = Path(db_path or settings.db_path)
    initialize_database(actual_db)
    worker = BackgroundWorker(actual_db)
    worker_enabled = settings.agent_worker_mode != "external" if start_worker is None else start_worker
    ...
    async def security_headers(request: Request, call_next):
        if request.method in {"POST", "PUT", "PATCH", "DELETE"}:
            ...
            same_origin = bool(origin and origin.rstrip("/").endswith(f"://{request.headers.get('host', '')}"))
            if origin and origin.rstrip("/") not in settings.allowed_origins and not same_origin:
        ...
        response.headers["Content-Security-Policy"] = "default-src 'self'; img-src 'self' data:; style-src 'self' 'unsafe-inline'; script-src 'self'; connect-src 'self'; object-src 'none'; base-uri 'self'; form-action 'self'"
    ...
    def health(request: Request):
        ...
        try:
            ...
            return {"status": "ok" if not fk else "degraded", "database": "ok", "foreign_key_violations": len(fk), "queued_jobs": queued, "food_data_ready": normalized > 0, "worker": "in_process" if worker_enabled else "external", "version": "2.0.0"}
        finally:
            conn.close()
    ...
    def chat(payload: ChatRequest, request: Request, user: dict = Depends(current_user)):
        with transaction(request.app.state.db_path, immediate=True) as conn:
            ...
            if settings.agent_worker_mode != "external":
                ...
                return Response(content=json.dumps({"job_id": job_id, "status": "queued"}), media_type="application/json", status_code=202)
    ...
    def link_code(request: Request, payload: TelegramLinkCodeRequest | None = None, user: dict = Depends(current_user)):
        code, digest = new_link_code()
        now = datetime.now(timezone.utc)
        expires = now + timedelta(minutes=10)
        with transaction(request.app.state.db_path, immediate=True) as conn:
            child_id = payload.child_id if payload else None
            if child_id:
                child = conn.execute("SELECT id FROM child_profiles WHERE id=? AND user_id=?", (child_id, user["id"])).fetchone()
            else:
                child = conn.execute("SELECT id FROM child_profiles WHERE user_id=? ORDER BY created_at LIMIT 1", (user["id"],)).fetchone()
            if not child:
                raise HTTPException(422, "Pilih atau buat profil anak sebelum menautkan Telegram")
            child_id = child["id"]
            ...
            conn.execute("INSERT INTO channel_link_codes(id,user_id,child_id,code_hash,created_at,expires_at) VALUES(?,?,?,?,?,?)", (new_id("link"), user["id"], child_id, digest, now.isoformat(timespec="milliseconds"), expires.isoformat(timespec="milliseconds")))
        return {"code": code, "child_id": child_id, "expires_at": expires.isoformat(timespec="milliseconds"), "single_use": True}
    ...
    def dashboard(request: Request, user: dict = Depends(current_user)):
        ...
        try:
            ...
            for child in children_rows:
                latest = conn.execute("SELECT id,measured_at,weight_kg,height_cm,head_circumference_cm,notes FROM measurements WHERE child_id=? ORDER BY measured_at DESC LIMIT 1", (child["id"],)).fetchone()
                child["latest_measurement"] = dict(latest) if latest else None
            if children_rows:
                measurements = [dict(x) for x in conn.execute("SELECT id,child_id,measured_at,weight_kg,height_cm,head_circumference_cm,notes FROM measurements WHERE child_id=? ORDER BY measured_at DESC LIMIT 24", (children_rows[0]["id"],))]
            ...
            return {"user": user, "children": children_rows, "measurements": measurements, "latest_runs": [run_payload(conn, x) for x in latest_ids], "pending_jobs": jobs, "food_stats": food_stats_payload(conn), "telegram": telegram_status(conn, user["id"])}
        finally:
            conn.close()
