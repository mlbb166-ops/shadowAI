...
export async function api<T = unknown>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  try {
    ...
    throw new ApiError('Layanan belum dapat dihubungi. Pastikan server lokal sedang berjalan.', 0)
  }
...
export const endpoints = {
  ...
  register: (name: string, email: string, password: string, acceptDataProcessing: boolean) => api('/auth/register', { method: 'POST', body: JSON.stringify({ display_name: name, email, password, accept_data_processing: acceptDataProcessing }) }),
