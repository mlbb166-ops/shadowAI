...
from app.agent import run_until_idle
from app.db import connect, initialize_database, transaction
from app.main import create_app
from app.pipeline import import_food_data
from app.security import iso_now, new_id
from app.telegram import consume_link_code, handle_update
...
def test_link_code_single_use_and_telegram_dedup(client: TestClient, db_path: Path):
    user = register(client)
    code = client.post("/api/telegram/link-code").json()["code"]
