...
from .security import iso_now, new_id
...
def claim_next_job(db_path: Path | str | None = None) -> dict | None:
    with transaction(db_path, immediate=True) as conn:
        row = conn.execute("SELECT * FROM agent_jobs WHERE status='queued' AND available_at<=? ORDER BY created_at LIMIT 1", (iso_now(),)).fetchone()
        if not row:
            return None
...
def process_one_job(db_path: Path | str | None = None) -> dict | None:
    ...
    return run_job(job, db_path) if job else None
